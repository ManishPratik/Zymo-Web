...
export const fetchAllTestZtCollections = async function (
  ...
  try {
    ...
    for (const collectionName of cityCollections) {
      ...
      if (foundCars && collectionName !== "testZt") continue;

      try {
        // Process each city separately
        for (const city of cityVariations) {
          try {
            // Explicitly access the "Cars" subcollection under each city
            ...
            const cars = cityDoc.docs.map((doc) => {
              ...
              const perHourRate = isAbove24Hours
                ? basePrice / 24
                : parseInt(carData.below24HourRate) || 0;
              ...
              return {
                ...
                securityDeposit:
                  parseInt(carData["Security Deposit"]) ||
